...
There are two possible payloads that can be returned from this endpoint. The difference between the two is whether or not the cloud account has an un-viewed password for the remote user. All passwords in Nexcess are stored as encrypted 'datashares'. In the case of the password for the remote user, it can only be viewed once. Once viewed, it is deleted. If the password for the remote user has already been viewed the this is the payload that will be returned.
